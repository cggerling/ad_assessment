...
.EXAMPLE
    .\AD-Analyse-V5.ps1 -Verzeichnis "D:\Reports" -KeineKonsole
    Report nach D:\Reports schreiben, ohne Konsolenausgabe.

.EXAMPLE
    .\AD-Analyse-V5.ps1 -Bereiche @{ dnschk = 0; allgpo = 0 }
    Kompletter Lauf, aber ohne DNS- und GPO-Pruefung.

.EXAMPLE
    .\AD-Analyse-V5.ps1 -Vergleich "C:\AD-Assessment\DC01\AD-Analyse-Report_alt.json"
...
$CheckKatalog = @{
    ...
    'kerberoasting' = @{
        ...
        Hintergrund = 'Dienste (z. B. SQL, Web, Exchange) laufen unter sogenannten Dienstkonten, die über einen Dienstnamen (Service Principal Name, SPN) im Verzeichnis eindeutig zugeordnet sind. Kerberos erlaubt es jedem bereits angemeldeten Konto, für einen solchen Dienst ein Service-Ticket anzufordern - das ist normales Verhalten und erfordert keine besonderen Rechte. Ein Teil dieses Tickets ist mit einem Schlüssel verschlüsselt, der direkt aus dem Passwort des Dienstkontos abgeleitet wird. Der Anforderer kann das Ticket speichern und das Passwort anschließend in aller Ruhe auf eigener Hardware durchprobieren - ganz ohne weiteren Kontakt zum Domänencontroller. Deshalb entsteht weder eine Kontosperrung noch ein Alarm. Ist das Dienstkonto-Passwort schwach oder wird es selten geändert, fällt es in Minuten bis Stunden; je höher die Rechte des Kontos, desto größer der Schaden.'
        Beispiel = 'Ein Angreifer, der bereits ein einfaches Mitarbeiterkonto übernommen hat (etwa über eine Phishing-Mail), fragt gezielt das Service-Ticket des Dienstkontos "svc-sql" ab, speichert es und lässt das Passwort über Nacht auf einem leistungsstarken Rechner durchprobieren. Da "svc-sql" aus Bequemlichkeit in der Gruppe "Domain Admins" liegt, übernimmt der Angreifer damit die gesamte Domäne - ohne je selbst erhöhte Rechte besessen zu haben.'
